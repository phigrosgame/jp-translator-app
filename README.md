...
- 🇯🇵 **日語語音辨識**：Vosk 離線日語語音識別（可選快速小模型或高精度大模型）
- 🌐 **離線翻譯**：Google MLKit 離線日譯中，不需要任何 API Key
- 🈶 **繁體字幕**：翻譯結果自動簡轉繁（OpenCC）
- 📺 **懸浮字幕**：字幕懸浮在所有 APP 上方，可拖動位置
- 📜 **翻譯歷史**：保存翻譯記錄，可回看
- ⚡ **低延遲**：串流辨識，講話停頓即出字幕；長句最多約 6 秒強制斷句
...
APP 首次啟動時會自動下載：
- 日語語音識別模型（Vosk，快速小模型約 50MB）
- 日語→中文翻譯模型（MLKit，約 25MB）
...
主畫面可選擇語音辨識模型：
- **快速（小）** `vosk-model-small-ja-0.22`，約 50MB，預設。
- **高精度（大）** `vosk-model-ja-0.22`，約 1GB，辨識明顯更準。切到此項會下載大模型。
...
```
Android/data/com.jptranslator.app/files/models/
```

- 用 `adb install -r` 或 Android Studio 直接更新 App 時，模型會**保留，不用重下**。
- 想省下在手機下載 1GB 的時間，可在 PC 下載並解壓 [vosk-model-ja-0.22](https://alphacephei.com/vosk/models/vosk-model-ja-0.22.zip)，
  用 USB 把整個 `vosk-model-ja-0.22` 資料夾放進上面的 `models/` 目錄，開 App 選「高精度」即自動偵測載入。
...
**Q: 字幕被切太碎 / 一句話被切成好幾段？**
A: 講話中的短停頓（換氣、思考）會先累積合併，停夠久才整句翻譯。調大 `AudioCaptureService.kt` 的 `pauseToleranceChunks`（預設 15 ≈ 1.5 秒）可合併得更完整，但出字幕會慢一點。

**Q: 延遲太高、想更快出字幕？**
A: 反過來調小 `pauseToleranceChunks`；另外 `maxPendingChunks`（預設 150 = 15 秒）是「講太久沒停」的強制送出上限。

**Q: 想要簡體字幕？**
A: 拿掉 `TranslateHelper.kt` 中 `toTraditional(...)` 的呼叫，直接回傳 MLKit 的簡體結果即可。
